...
import { signInWithEmailAndPassword } from 'firebase/auth'; // ⚡ Added signIn import
import { 
...
    Image as ImageIcon, Mic, Trash2, Copy, Forward, StopCircle, Download, Reply, ChevronDown, LogIn, User
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

const ChatPage = ({ currentUser, initialChatId, onBack, onCallStart, onChatSelect, onSwitchAccount }) => {
...
  const [savedAccounts, setSavedAccounts] = useState([]); // ⚡ State for saved accounts
...
      if (!account.password) {
          alert("Login required for this account.");
          if (onSwitchAccount) onSwitchAccount();
          return;
      }

      try {
          // Decode password (simple base64 from Login.jsx logic)
          const savedPass = atob(account.password);
          await signInWithEmailAndPassword(auth, account.email, savedPass);
          
          // Update Local Storage for App.jsx to pick up
          localStorage.setItem('shayari_user', account.username);
          
          // Reload to refresh App state with new user
          window.location.reload();
      } catch (error) {
          console.error("Switch failed", error);
          alert("Failed to switch account. Please log in manually.");
          if (onSwitchAccount) onSwitchAccount();
      }
  };
...
                    className="flex items-center gap-1 text-xl font-bold font-sans text-gray-900 hover:opacity-70 transition group"
                >
                    {currentUser} 
                    <ChevronDown size={20} className={`transition-transform duration-200 ${isUserMenuOpen ? 'rotate-180' : ''}`} />
                </button>

                {/* ⚡ Dropdown Menu with Saved Accounts */}
                <AnimatePresence>
                    {isUserMenuOpen && (
                        <motion.div 
...
                                            >
                                                {acc.photoURL ? (
                                                    <img src={acc.photoURL} alt={acc.username} className="w-8 h-8 rounded-full object-cover" />
                                                ) : (
...
                            <button 
                                onClick={() => { setIsUserMenuOpen(false); if(onSwitchAccount) onSwitchAccount(); }} 
                                className="w-full text-left p-3 text-sm font-bold text-blue-500 hover:bg-gray-50 flex items-center gap-2"
                            >
                                <LogIn size={16} /> Log into an existing account
                            </button>
                        </motion.div>
                    )}
                </AnimatePresence>
...
            <button onClick={() => { setIsForwardMode(false); setIsSearchOpen(true); }} className="p-2 rounded-full hover:bg-gray-50 text-gray-900 transition">
                <Edit size={24} />
            </button>
        </div>
...
                        </div>
                        <button onClick={(e) => {e.stopPropagation();}} className="absolute right-4 opacity-0 group-hover:opacity-100 p-2 text-gray-400 hover:text-red-500 transition"><Trash2 size={18} /></button>
                    </div>
                ))
            )}
        </div>
